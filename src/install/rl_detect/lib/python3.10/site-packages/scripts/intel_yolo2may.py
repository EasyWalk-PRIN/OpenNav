...
class YOLODetectorNode(Node):
    ...
    def run(self):
        while True:
            ...
            self.image.fill(0)  # Reset occupancy map
            for box, score, class_id in zip(boxes, scores, class_ids):
                x1, y1, x2, y2 = box.astype(int)
                cv2.rectangle(self.image, (x1, y1), (x2, y2), (0, 255, 0), -1)  # Highlight detected object on map
                ...
                cv2.putText(self.image, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

            # Convert the occupancy map image to ROS Image message and publish
            out_msg = self.bridge.cv2_to_imgmsg(self.image, 'bgr8')
