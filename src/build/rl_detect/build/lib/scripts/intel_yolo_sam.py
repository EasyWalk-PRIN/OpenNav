...
from rl_detect.efficient_sam import load, inference_with_boxes
class YOLODetectorNode(Node):
    def __init__(self):
        ...
        self.EFFICIENT_SAM_MODEL = load(device=self.DEVICE)
    ...
    def run(self):
        while True:
            ...
            #print datatype of color_image
            print(color_image.dtype)
            #create a mask image of same datatype as color image and same size
            mask_image = np.zeros_like(color_image)
            for mask in masks:
                # Convert the mask to 0s and 1s
                mask = mask.astype(np.uint8)
                #convert mask to 3 channel
                mask = cv2.merge([mask, mask, mask])
                # Add the mask to the mask image
                mask_image = cv2.add(mask_image, mask)

            # Threshold the mask image to get a binary mask
            mask_image = (mask_image > 0).astype(np.uint8) * 255
            #imshow mask    
            cv2.imshow("Mask Image", mask_image)
            combined_img = self.drawer(color_image,mask_image, depth_image, boxes, scores, class_ids)
