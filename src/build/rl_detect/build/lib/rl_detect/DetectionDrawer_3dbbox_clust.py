...
class DetectionDrawer():
    ...
    def draw_detections(self, image, depth_image, boxes, scores, class_ids, mask_alpha=0.3):
        ...
        for box, score, label, color in zip(boxes, scores, classes, colors):
            ...
            object_depth = np.median(depth_image[int(y1):int(y2), int(x1):int(x2)])/10.0
            ...
            bbox_3d.append(bx3d)
        # detection = cv2.addWeighted(mask_img, mask_alpha, det_img, 1 - mask_alpha, 0)
        #save image
        # cv2.imwrite("/home/rameez/combined_img.jpg", image)
        # cv2.imwrite("/home/rameez/mask_img.jpg", mask_img)
        # cv2.imwrite("/home/rameez/det_img.jpg", detection)
...
#       Show color_image, depth_image, color_mask
        ...
        labels = np.array(pcd.cluster_dbscan(eps=0.08, min_points=100, print_progress=False))
        max_label = labels.max()
        biggest_cluster_indices = np.where(labels == max_label)[0]

        # Select points from pcd based on the indices
        pcd = pcd.select_by_index(biggest_cluster_indices)
        # max_label = labels.max()
        ...
        # import pdb
        # pdb.set_trace()
        # cl, ind = pcd.remove_radius_outlier(nb_points=100, radius=0.05)
        # pcd = pcd.select_by_index(ind)
        # pcd = pcd.voxel_down_sample(voxel_size=0.001)
        # pcd1, ind = pcd.remove_statistical_outlier(nb_neighbors=5, std_ratio=2.0)
